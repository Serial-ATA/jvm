...
pub(crate) fn generate_intrinsics(native_directory: &Path, generated_directory: &Path, modules: &[Module]) {
	...
	for module in modules {
		module.for_each_class(|class| {
			intrinsic_methods.extend(
				class
					.methods()
					.filter_map(|method| method.intrinsic_name(class).map(|id| (id, (module.name_for_class(&class.class_name), IntrinsicMethod::from(method))))),
			)
		});
	}

	// Generate any additional symbols that we need
	generate_symbols(native_directory, generated_directory, intrinsic_methods.values());
	...
	writeln!(
		...
		create_intrinsic_name_table(std::iter::once("None").chain(intrinsic_methods.keys().map(String::as_str)), total_ids)
	)
	.unwrap();
	...
	writeln!(
		&mut generated_file,
		"{}",
		create_intrinsic_id_enum(std::iter::once("None").chain(intrinsic_methods.keys().map(String::as_str)))
	)
	.unwrap();

	writeln!(&mut generated_file, "{}", create_method_mappings(intrinsic_methods.iter())).unwrap();
}

/// Generates additional symbols, injecting them into the `vm_symbols::define_symbols!` call
/// in `runtime/src/symbols.rs`
fn generate_symbols<'a>(native_directory: &Path, generated_directory: &Path, iter: impl Iterator<Item = &'a (String, IntrinsicMethod)> + Clone) {
	...
	generate_class_name_symbols(generated_directory, &mut symbols_file_contents, iter.clone());
	generate_method_name_symbols(generated_directory, &mut symbols_file_contents, iter.clone());
	generate_method_signature_symbols(generated_directory, &mut symbols_file_contents, iter);

	let mut symbols_file = OpenOptions::new().truncate(true).write(true).open(&symbols_file_path).unwrap();
	symbols_file.write_all(symbols_file_contents.as_bytes()).unwrap();
}
...
fn get_position_of_marker_comment(contents: &str, section_marker_comment: &str) -> usize {
	contents.rfind(section_marker_comment).expect(&format!("Failed to find marker comment: {}", section_marker_comment)) + section_marker_comment.len()
}

fn check_in_section(contents: &str, section_header_position: usize, marker_comment_position: usize, check_for: &str) -> bool {
	contents[section_header_position..marker_comment_position].contains(&format!("\"{}\"", check_for))
}
...
/// }
/// ```
fn generate_class_name_symbols<'a>(generated_directory: &Path, symbols_file_contents: &str, iter: impl Iterator<Item = &'a (String, IntrinsicMethod)>) {
	const SECTION_HEADER: &str = "// Classes";
	const MARKER_COMMENT: &str = "// -- GENERATED CLASS NAME MARKER, DO NOT DELETE --";

	let section_header_position = get_position_of_marker_comment(symbols_file_contents, SECTION_HEADER);
	let marker_comment_position = get_position_of_marker_comment(symbols_file_contents, MARKER_COMMENT);
	...
	let mut class_name_symbols_file = OpenOptions::new().write(true).truncate(true).create(true).open(generated_directory.join("class_names.symbols")).unwrap();

	for class_name in class_names_to_add {
		if !check_in_section(symbols_file_contents, section_header_position, marker_comment_position, class_name) {
			writeln!(class_name_symbols_file, "{}: {:?},", class_name.replace('/', "_"), class_name).unwrap();
		}
	}
}
...
/// }
/// ```
fn generate_method_name_symbols<'a>(generated_directory: &Path, symbols_file_contents: &str, iter: impl Iterator<Item = &'a (String, IntrinsicMethod)>) {
	...
	let mut method_name_symbols_file = OpenOptions::new().write(true).truncate(true).create(true).open(generated_directory.join("method_names.symbols")).unwrap();

	for (name_symbol, generated_name) in methods_to_add {
		if !check_in_section(symbols_file_contents, section_header_position, marker_comment_position, generated_name) {
			writeln!(method_name_symbols_file, "{}: {:?},", name_symbol, generated_name).unwrap();
		}
	}
}
...
/// }
/// ```
fn generate_method_signature_symbols<'a>(generated_directory: &Path, symbols_file_contents: &str, iter: impl Iterator<Item = &'a (String, IntrinsicMethod)>) {
	...
	let mut signature_symbols_file = OpenOptions::new().write(true).truncate(true).create(true).open(generated_directory.join("signature_names.symbols")).unwrap();

	for (signature_symbol, descriptor) in signatures_to_add {
		if !check_in_section(symbols_file_contents, section_header_position, marker_comment_position, descriptor) {
			writeln!(signature_symbols_file, "{}: {:?},", signature_symbol, descriptor).unwrap();
		}
	}
}

fn create_intrinsic_name_table<'a>(intrinsic_ids: impl Iterator<Item = &'a str>, total_ids: usize) -> String {
...
fn create_method_mappings<'a>(intrinsic_ids: impl Iterator<Item = (&'a String, &'a (String, IntrinsicMethod))>) -> String {
	let mut intrinsic_id_method_mapping = String::from(r#"
impl IntrinsicId {
...
"#);
	...
	for (id, (class_name, method)) in intrinsic_ids {
		writeln!(intrinsic_id_method_mapping, "\t\t\tid3 if id3 == intrinsics_id3!(sym!({}), sym!({}), sym!({})) => {{ if IntrinsicFlags::from(flags) == {} {{ return sym!({}) }} }}", class_name.replace('/', "_"), method.name_symbol, method.signature_symbol, method.intrinsic_flags, id).unwrap();
	}

	writeln!(intrinsic_id_method_mapping, "\t\t\t_ => return IntrinsicId::None,\n\t\t}}\n\n\t\treturn IntrinsicId::None;\n\t}}\n}}").unwrap();
