...
use crate::constant_pool::types::{
	self, ConstantPoolEntryError, LoadableConstantPoolValue, MethodHandleEntry,
};
...
impl ResolvedAnnotation {
	pub(crate) fn resolve_from(
	...
	) -> Result<Self, ConstantPoolEntryError> {
		...
		Ok(Self {
			name: String::from_utf8(name.to_vec()).unwrap(), // TODO: Error handling
			element_value_pairs,
		})
	}
}
...
impl ResolvedElementValuePair {
	fn resolve_from(
		raw_value_pair: &ElementValuePair,
		constant_pool: &ConstantPool,
	) -> Result<Self, ConstantPoolEntryError> {
		let element_name =
...
impl ResolvedElementValue {
	fn resolve_from(
		raw_element_value: &ElementValue,
		constant_pool: &ConstantPool,
	) -> Result<Self, ConstantPoolEntryError> {
		let tag = raw_element_value.tag;

		let value;
		match &raw_element_value.ty {
			ElementValueType::Byte { const_value_index } => {
				value = constant_pool
					.get::<types::raw::Integer>(*const_value_index)
					.map(ResolvedElementValueType::Byte)?;
			},
			ElementValueType::Char { const_value_index } => {
				value = constant_pool
					.get::<types::raw::Integer>(*const_value_index)
					.map(ResolvedElementValueType::Char)?;
			},
			ElementValueType::Double { const_value_index } => {
				value = constant_pool
					.get::<types::raw::Double>(*const_value_index)
					.map(ResolvedElementValueType::Double)?;
			},
			ElementValueType::Float { const_value_index } => {
				value = constant_pool
					.get::<types::raw::Float>(*const_value_index)
					.map(ResolvedElementValueType::Float)?;
			},
			ElementValueType::Int { const_value_index } => {
				value = constant_pool
					.get::<types::raw::Integer>(*const_value_index)
					.map(ResolvedElementValueType::Int)?;
			},
			ElementValueType::Long { const_value_index } => {
				value = constant_pool
					.get::<types::raw::Long>(*const_value_index)
					.map(ResolvedElementValueType::Long)?;
			},
			ElementValueType::Short { const_value_index } => {
				value = constant_pool
					.get::<types::raw::Integer>(*const_value_index)
					.map(ResolvedElementValueType::Short)?;
			},
			ElementValueType::Boolean { const_value_index } => {
				value = constant_pool
					.get::<types::raw::Integer>(*const_value_index)
					.map(ResolvedElementValueType::Boolean)?;
			},

			ElementValueType::String { const_value_index } => {
				let raw = constant_pool.get::<types::raw::RawConstantUtf8>(*const_value_index)?;
				value = ResolvedElementValueType::String(String::from_utf8(raw.to_vec()).unwrap()); // TODO: Error handling
			},
			ElementValueType::Enum {
				...
				let type_name =
					constant_pool.get::<types::raw::RawConstantUtf8>(*type_name_index)?;
				let const_value =
					constant_pool.get::<types::raw::RawConstantUtf8>(*const_value_index)?;
				value = ResolvedElementValueType::Enum {
					type_name: String::from_utf8(type_name.to_vec()).unwrap(), /* TODO: Error handling */
					const_value: String::from_utf8(const_value.to_vec()).unwrap(), /* TODO: Error handling */
				};
			},
			...
			ElementValueType::Annotation { annotation } => {
				...
				value = ResolvedElementValueType::Annotation { annotation };
			},
			ElementValueType::Array { values } => {
				let values = values
					...
					.collect::<Result<Vec<_>, _>>()?;
				value = ResolvedElementValueType::Array { values };
			},
		}
...
impl<'a> ResolvedInnerClass<'a> {
	pub(crate) fn resolve_from(
		...
		let inner_class =
			constant_pool.get::<types::raw::RawClassName>(raw.inner_class_info_index)?;
