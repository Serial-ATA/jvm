...
use syn::{Abi, FnArg, ItemFn, Pat, PatIdent, Path, ReturnType, Type, TypePath, parse_macro_input};
	...
	(
		...
		impl SafeJniWrapperType {
			...
			fn to_safe(self) -> proc_macro2::TokenStream {
				match self {
					SafeJniWrapperType::Primitive(p) => self.to_raw(),
					$(
					SafeJniWrapperType::$obj_ty => {
			...
			fn safe_conversion_fn(self, param_name: Ident) -> proc_macro2::TokenStream {
				match self {
					SafeJniWrapperType::Primitive(p) => p.as_ident().to_token_stream(),
					$(
					SafeJniWrapperType::$obj_ty => {
...
impl Error {
	fn into_syn(self, span: Span) -> syn::Error {
		match self {
...
pub fn jni_call(_attr: TokenStream, input: TokenStream) -> TokenStream {
	...
	let extern_fn = generate_extern_fn(&input, params, return_ty);
	quote! {
		#(#errors)*
...
fn generate_extern_fn(
	...
	quote! {
		...
		pub extern "system" fn #fn_name(env: *mut ::jni::sys::JNIEnv, #(#sys_params),*) #ret {
			...
			super::#fn_name(env, #(#all_param_names)*)
		}
	}
}
