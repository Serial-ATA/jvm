...
impl StringInterner {
    ...
    pub fn intern_string(raw: &[u1], thread: &ThreadRef) -> ClassInstanceRef {
        ...
        let reference_to_char_array = Operand::Reference(Reference::Array(ArrayInstance::new(char_array_class, ArrayContent::Char(chars_utf16))));

        let new_java_string_instance = ClassInstance::new(Arc::clone(&java_string_class));

        // public String(char[] value)
        let constructor_args = vec![Operand::Reference(Reference::Class(Arc::clone(&new_java_string_instance))), reference_to_char_array];

        Class::construct(&java_string_class, thread, STRING_CONSTRUCTOR_FROM_CHAR_ARRAY, constructor_args);
